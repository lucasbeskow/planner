...
6. Update the ticket status and acceptance evidence only after the checks pass. The evidence records `harness`, `model`, `effort`, `tokens`, `completed_at`, and `validation`; use `unknown` or a justification instead of inventing values (see `docs/prd.md`, "Fechamento de tickets").
